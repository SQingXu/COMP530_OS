                                ...
                                size_t count, loff_t *ppos)
{
  ...
  cur_parent_pid = task_pid_nr(current->real_parent);
  
  /*get current process's info in struct task_struct*/
  cur_state = current->state;
  cur_flags = current->flags;
  cur_normal_prio = current->normal_prio;

  /*get current process's info in struct mm_struct*/
  if(current->mm == NULL){
    /*if current process's mm pointer is NULL*/
    strcpy(respbuf, "Failed: pointer to struct mm_struct is null\n");
    printk(KERN_DEBUG "getpinfo: call %s will return %s\n", callbuf, respbuf);
    preempt_enable();
    return count;
  }
  cur_map_count = current->mm->map_count;
  cur_shared_vm = current->mm->shared_vm;
  cur_exec_vm = current->mm->exec_vm;
  cur_stack_vm = current->mm->stack_vm;
  cur_total_vm = current->mm->total_vm;
